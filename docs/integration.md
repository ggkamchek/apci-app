...
| Контракты | `shared/proto/apci/security/v1/` |
...
Статус: запланировано. Контракт и обработчик Центра готовы, публикация события из Chat API ещё не подключена.
...
Статус: запланировано. Сейчас Chat API сохраняет ciphertext, но событие `message.sent` в Центр пока не публикует.
...
| Presence (в сети / не в сети) | UX; для графа связей не используется |
| Typing («печатает») | UX; для графа связей не используется |
| Поиск пользователей | Планируется на этапе 3, см. [roadmap.md](./roadmap.md) |
...
| Retry | Планируется: outbox-таблица в БД чатов + фоновый воркер |
...
Для админки используется встроенный web UI в `security-center/internal/ui/web/`.

---

## Docker / локальный запуск

Сейчас локальная среда запускается через `scripts/dev.ps1`: Docker поднимает две PostgreSQL, а Chat API и Центр Безопасности стартуют через `go run` в отдельных окнах.

Полностью контейнеризованный запуск `chat-api` + `security-center` — отдельная инфраструктурная задача.
